...
export const PropertySearch = () => {
  ...
  const handlePageClick = async (pageNumber) => {
    ...
    await router.push(`${router.query.slug.join('/')}?page=${pageNumber}&petFriendly=${petFriendly}&hasParking=${hasParking}&minPrice=${minPrice}&maxPrice=${maxPrice}`, 
    null , {
      shallow: true,
    })
    search();
  ...
  const handleSearch = async ({petFriendly,hasParking,minPrice,maxPrice}) => {
    ...
    await router.push(`${router.query.slug.join('/')}?page=$1&petFriendly=${petFriendly === "true"}&hasParking=${hasParking === "true"}&minPrice=${minPrice}&maxPrice=${maxPrice}`, null , {
      shallow: true,
    })
    search()
  }
